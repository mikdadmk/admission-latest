...
//     // Handle File Uploads using /tmp (ephemeral storage)
//     // Note: Files here will be deleted once the function terminates.
//     const uploadFolder = path.join("/tmp", email);
//     await fs.mkdir(uploadFolder, { recursive: true });
...
//         const fileBuffer = await file.arrayBuffer();
//         await fs.writeFile(filePath, Buffer.from(new Uint8Array(fileBuffer)));
//         filePaths[key] = `/tmp/${email}/${file.name}`;
//         console.log(`File for ${key} stored at ${filePaths[key]}`);
//       } else {
//         console.log(`No file received for ${key}`);
//       }
//     }
...
//       name, fatherName, motherName, guardianName, relation,
//       address, dob, phone, whatsapp, email,
//       files: filePaths, // Store file paths (remember these are in /tmp)
//       createdAt: new Date()
//     });
...
export async function POST(request) {
  try {
    ...
    const publicUploadFolder = path.join(process.cwd(), "public", "uploads", email);
    ...
    for (const key of fileFields) {
      const file = formData.get(key);
      if (file && file.name) {
        const filePath = path.join(publicUploadFolder, file.name);
        const fileBuffer = await file.arrayBuffer();
        await fs.writeFile(filePath, Buffer.from(new Uint8Array(fileBuffer)));
